...
test('health is public while MCP requires a bearer token', async () => {
  ...
  try {
    ...
    assert.equal(health.bridgeVersion, '0.3.5');
...
test('extension relay is prewarmed on its dedicated port and validates the token', async () => {
  ...
  try {
    ...
    const handshake = 'token=extension-secret&extensionVersion=0.3.4&extensionProtocol=2&capabilityVersion=1';
  ...
  try {
    ...
    const accepted = await fetch(bridge.url('/extension-status'), {
      ...
      body: JSON.stringify({ connected: true, profile: 'current', tabCount: 7,
        extensionVersion: '0.3.4', extensionProtocol: 2, capabilityVersion: 1,
    ...
    const staleProfile = await fetch(bridge.url('/extension-status'), {
      method: 'POST',
      headers: { authorization: 'Bearer extension-secret', 'content-type': 'application/json' },
      body: JSON.stringify({ connected: false, extensionVersion: '0.3.0', extensionProtocol: 2 }),
    });
    assert.equal(staleProfile.status, 204);
    assert.equal((await waitForHealth(bridge.url('/health'), 'ready')).connected, true);
  } finally {
    await bridge.close();
  }
});
